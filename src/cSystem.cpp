...
namespace MultiColSLAM
{
		...
		mbDeactivateLocalizationMode(false)
	{
		...
		LoadCmpKFMs(path2MCScalibrationFiles);
		LoadUltrasonicData(path2MCScalibrationFiles);
		...
		cMultiCamSys_& camSystem)
	{
		...
		for (int c = 0; c < nrCams; ++c)
		{
			...
			for (int p = 0; p < 12; ++p)
			{
				string param = "CameraSystem.cam" + to_string(c) + "_" + to_string(p);
				tmp[p] = mcs_calib_data[param];
			}

			cv::Matx<double, 4, 4> homM(
			tmp[0], tmp[3], tmp[6], tmp[9],
			tmp[1], tmp[4], tmp[7], tmp[10],
			tmp[2], tmp[5], tmp[8], tmp[11],
			0.0, 0.0, 0.0, 1.0);
			...
			for (int i = 0; i < nrinvpol; ++i)
			{
				...
				cout << endl << "nrinvpol" << i << ":"<< (float)fSettings["Camera.pol" + to_string(i)]<<endl << endl;
			}
			...
			if (createMirrorMasks == 1)
				CreateMirrorMask(camModel, c, 4, mirrorMasks);
			else
    ...
    void cSystem::LoadCmpKFMs(const string &strDataPath)
    {
        // 读关键帧编号
        string strFilePath = "MKFTrajectory_Veh.txt"; //strDataPath + "MKFTrajectory_Veh.txt";
        cout << endl << "Load compare Vehicle MKF trajectory from: " << strFilePath << " ..." << endl;

        ifstream fin( strFilePath );
        std::vector<cv::Matx44d> cmpKFMs;
        while( fin.peek() != EOF )
        {
            float index_keyframe;
            ...
            Eigen::Isometry3d t(q.matrix().transpose());
            t(0,3) = data[0]; t(1,3) = data[1]; t(2,3) = data[2];
            // cout << "Matrix44 cmpare KFM :" << t.matrix() << endl;

            cv::Matx44d cmpKFMr;
            for(int i=0;i<4;i++)
                for(int j=0; j<4; j++)
                    cmpKFMr(i,j) = t(i,j);
            // cout << "Matrix44 cmpare KFM :" << cmpKFMr << endl;
            cmpKFMs.push_back( cmpKFMr );
        }
        fin.close();

        if (mpMapPublisher)
        {
            mpMapPublisher->setCmpKFM( cmpKFMs );
        }
    }

	//load data for ultrasonic waves
	void cSystem::LoadUltrasonicData(const string &strParaPath)
	{
		string strDataPath = "DataSet/UltrasonicData/UltrasonicData1229PC.txt";

		cout << endl << "Load Ultrasond data from: " << strDataPath << " ..." << endl;

        ifstream fin( strDataPath );
		if (!fin)
		{
			cout << " Ultrasond File is not exist! "<< endl;
			fin.close();
			return;
		}
		string line;
		...
		posS = line.find("X: ")+3;
		posE = line.find(",",0);
		tempStr = line.substr(posS,posE-posS);
		VehiclePos(0) = atof(tempStr.c_str());

		posS = posE + 5;
		posE = line.find(",", posS);
		VehiclePos(1) = atof(line.substr(posS,posE-posS).c_str());
		VehiclePos(2) = 0.0;
		// std::istringstream iss(line);
		// iss >> Str >> Str >> Str >> Str >> Str >> Str >> strX >> Str >> strY;

		std::getline(fin, line);
		posS = line.find(":")+1;
		int iCount = atoi(line.substr(posS,line.length()-posS).c_str());
		int iCnt = 0;

    	std::vector<cv::Vec3d> USData;
		std::vector<int> USTypes;
		USData.reserve(iCount);
		USTypes.reserve(iCount);
		while (std::getline(fin, line))
		{
			posS = line.find("real: ")+6;
			posE = line.find(", point");
			int Type = atoi(line.substr(posS,posE-posS).c_str());
			USTypes.push_back(Type);

			posS = posE + 8;
			posE = line.find(",",posS);
			double PtX = atof(line.substr(posS,posE-posS).c_str()) * 0.01;
			PtX  = PtX + VehiclePos(0)-3.6;

			posS = posE + 1;
			posE = line.find(")");
			double PtY = atof(line.substr(posS,posE-posS).c_str()) * 0.01;
			PtY  = PtY + VehiclePos(1)-0.29;

			USData.push_back( cv::Vec3d(PtX,PtY,0.0) );
			iCnt += 1;
		}

        if (mpMapPublisher)
        {
            mpMapPublisher->setUltrasonicData( USData, USTypes);
        }
	}
}
	...
	const double& timestamp)
{
	...
	std::cout << endl << "---mState: WORKING imgSet.size():" << imgSet.size() <<endl << endl;
	convertedImages = imgSet;

	if (mState == WORKING || mState == LOST)
	{

		unsigned long startTime = cTimer::getCostTimeStart();
		std::cout << endl << "---mState: WORKING" << endl << endl;
		mCurrentFrame = cMultiFrame(convertedImages,
		timestamp, mp_mdBRIEF_extractorOct, mpORBVocabulary,
		camSystem, imgCounter - 1);
		printf("%s:%d\n",__FILE__,__LINE__ );
		cTimer::getCostTimeEnd(startTime);
	}
	else
	{
		unsigned long startTime = cTimer::getCostTimeStart();
		std::cout << endl << "----mState:" << mState << endl << endl;
		mCurrentFrame = cMultiFrame(convertedImages,
...
cout << "Initialize" << endl;
    ...
    {
		mpMapPublisher->SetInitMC(leadingCam); // for draw obstcale map
...
cout << "Initialize  CreateInitialMap" << endl;
...
void cTracking::CreateInitialMap(cv::Matx33d &Rcw, cv::Vec3d &tcw, int leadingCam)
{
    ...
    {
        Reset();
        cout << "TrackedMapPoints" << pKFcur->TrackedMapPoints()<<endl;
        return;
    }
...
bool cTracking::TrackLocalMap()
{
	...
	if (mCurrentFrame.mnId < mnLastRelocFrameId + mMaxFrames && mnMatchesInliers < 14)
	{
		cout <<"mnMatchesInliers:" << mnMatchesInliers <<" mCurrentFrame.mnId:" << mCurrentFrame.mnId  <<" mnLastRelocFrameId:" << mnLastRelocFrameId<<"  mMaxFrames:"<< mMaxFrames<< endl;
        return false;
	}
...
bool cTracking::Relocalisation()
{
	...
	if (!RelocalisationRequested())
        vpCandidateKFs = mpKeyFrameDB->DetectRelocalisationCandidates(&mCurrentFrame);//挑选候选关键帧
